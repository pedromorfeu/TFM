...
a = c(1,2,3,4, 1.1,2.2,3.1,4.1)
b = c(3,4,5,2, 3.2,4.1,5.1,2.1)
#b = c(3,4,5,2,5.2, 3.2,4.1,5.1,2.1,8.2)
label = c(1,1,1,1,2,2,2,2)
ab = data.frame(a, b, label)
ab
...
nlevels(factor(ab[, 3])) ## Number of groups. 
table(factor(ab[, 3])) ## Vector of groups-size. 
BoxMTest(ab[, -3], factor(ab[, 3]))

# p-value > 0.05 --> no significant differences between the covariance matrices
# p-value < 0.05 --> there are significant differences between the covariance matrices
# example Box's M statistic test: Box's M value=4.58 is not significant --> variances equal
